...
4b. **Workflow-step staffing** — satisfiability of a workflow isn't just "the actor role exists": FLAG
   any workflow step whose actor role will have **no assignable members at go-live** (an empty role
   means no assignee, so `submit` fails outright — the flow can't progress). Recommend staffing the
   role or designating a fallback assignee (creator/admin) before generate.
